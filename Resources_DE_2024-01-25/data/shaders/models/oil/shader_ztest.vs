...
SamplerState g_VertexTexture_sampler;
          ...
          out PSInput PSIn) 
{
	...
	float pixel = pow(g_VertexTexture.SampleLevel(g_VertexTexture_sampler, VSIn.UV, 0).x, 2.0f);
	float alpha = 1.0f - pixel;
	float luminosity = max((dot(normal, g_LightPosition.xyz) * 0.8f + 0.4f) * g_EnableLight + (1.0f - g_EnableLight), 0.2f);
	PSIn.Color = clamp(g_BodyLight * alpha * float4(luminosity, luminosity, luminosity, 1.0f), 0.0f, 1.0f);
	PSIn.Color.a = alpha;
	PSIn.UV = VSIn.UV + g_BlendTexCoord;
	
	PSIn.PositionModelView = mul(float4(position, 1.0f), g_Model).xzyw;
#if SHADOW_OUTPUT == 0 && USE_SHADOW == 1
    float4 LightSpacePos = mul( PSIn.PositionModelView, g_LightAttribs.ShadowAttribs.mWorldToLightView);
	...
	PSIn.Position = mul(PSIn.PositionModelView, g_ViewProj);
}