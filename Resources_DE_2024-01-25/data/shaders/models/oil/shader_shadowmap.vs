          ...
          out PSInput PSIn) 
{
	...
	PSIn.Position = mul(float4(position, 1.0f), g_Model);
	PSIn.Position -= g_BodyOrigin;
	PSIn.Position.x = PSIn.Position.z * (PSIn.Position.x + 2000.f) / (PSIn.Position.z - 4000.f);
	PSIn.Position.z = 5.0f;
	PSIn.Position += g_BodyOrigin;
	PSIn.Position = PSIn.Position.xzyw;
#if SHADOW_OUTPUT == 0 && USE_SHADOW == 1
