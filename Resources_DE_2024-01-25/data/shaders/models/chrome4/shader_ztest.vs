          ...
          out PSInput PSIn) 
{
	...
	float wave = fmod(g_WorldTime, 10000.0f) * 0.0001f;
	float3 L = float3(cos(g_WorldTime * 0.001f), sin(g_WorldTime * 0.002f), 1.0f);
	float w = dot(normal, L);
	PSIn.UV = float2(w, 1.0f - w) + float2(normal.y * 0.5f + L.y * 3.0f, -(normal.z * 0.5f + wave * 3.0f));
