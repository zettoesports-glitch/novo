...
{
	float4 tex1 = g_Textures.Sample(g_Textures_sampler, float3(PSIn.UV.xy, PSIn.Setting.x)) * PSIn.Color;
	float4 tex2 = g_Textures.Sample(g_Textures_sampler, float3(PSIn.UV.zw, PSIn.Setting.y)) * PSIn.Color;
	float4 color = float4(lerp(tex1.rgb, tex2.rgb, PSIn.Setting.z * step(0.25f, PSIn.Setting.z)), 1.0f);
#if USE_SHADOW == 1
	float3 normalWS = normalize(PSIn.NormalWS);
    float nDotL = max(saturate(dot(normalWS, -g_LightAttribs.f4Direction.xyz)), 0.6f);

    FilteredShadow Shadow;
    float CameraSpaceZ = PSIn.Position.w;
# if SHADOW_MODE == SHADOW_MODE_PCF
        Shadow = FilterShadowMap(g_LightAttribs.ShadowAttribs, g_tex2DShadowMap, g_tex2DShadowMap_sampler,
                                 PSIn.PosInLightViewSpace, ddx(PSIn.PosInLightViewSpace), ddy(PSIn.PosInLightViewSpace),
                                 CameraSpaceZ);
#else
        Shadow = SampleFilterableShadowMap(g_LightAttribs.ShadowAttribs, g_tex2DFilterableShadowMap, g_tex2DFilterableShadowMap_sampler,
                                           ...
                                           CameraSpaceZ);
#endif
	color.rgb *= (nDotL * g_LightAttribs.f4Intensity.xyz * max(Shadow.fLightAmount, g_LightAttribs.f4AmbientLight.w) + g_LightAttribs.f4AmbientLight.xyz);
#ifdef DSHADOW_COLOR
	color.rgb = lerp(color.rgb, float3(Shadow.fLightAmount, Shadow.fLightAmount, Shadow.fLightAmount), 0.99f);
	color.rgb = lerp(color.rgb, GetCascadeColor(Shadow), 0.1f);
#endif
#endif
	PSOut.Color = SRGBtoLINEAR(color);
}