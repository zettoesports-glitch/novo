...
void CCustomQuiz::ProcState_EMPTY(CUSTOM_QUIZ_INFO* lpInfo) // OK
{
	if(lpInfo->RemainTime > 0 && lpInfo->RemainTime <= (lpInfo->RuleInfo.AlarmTime*60))
	{
		if((lpInfo->AlarmMinSave=(((lpInfo->RemainTime%60)==0)?((lpInfo->RemainTime/60)-1):(lpInfo->RemainTime/60))) != lpInfo->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(745),lpInfo->RuleInfo.Name,(lpInfo->AlarmMinLeft+1));
		}
	}
	...
	if(lpInfo->RemainTime <= 0)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(746),lpInfo->RuleInfo.Name);
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(747),lpInfo->RuleInfo.Name,CustomQuiz.Question);
...
void CCustomQuiz::ProcState_START(CUSTOM_QUIZ_INFO* lpInfo) // OK
{
	...
	if(lpInfo->RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(749),lpInfo->RuleInfo.Name);
...
void CCustomQuiz::CommandQuiz(LPOBJ lpObj,char* arg)   
{
	...
	if (this->GetInfo(this->IndexSelected,&CustomQuiz) == 1)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(748),this->m_CustomQuizInfo[this->IndexInfo].RuleInfo.Name,lpObj->Name);
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(751),this->m_CustomQuizInfo[this->IndexInfo].RuleInfo.Name,CustomQuiz.Answer);
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(750),this->m_CustomQuizInfo[this->IndexInfo].RuleInfo.Name);
		}
	}
