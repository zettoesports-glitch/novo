...
void CEventPvP::Clear()
{
	...
	{
		GCPKLevelSend(this->m_Player1Index, gObj[this->m_Player1Index].PKLevel, gObj[this->m_Player1Index].PKTime);
	}
	if (gObjIsConnectedGP(this->m_Player2Index) == 1)
	{
		GCPKLevelSend(this->m_Player2Index, gObj[this->m_Player2Index].PKLevel, gObj[this->m_Player1Index].PKTime);
	}
...
void CEventPvP::MainProc() // OK
{
	...
	{ 
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(799));
		...
		if (this->m_EventPvPEnter && this->m_Player1Index != -1 && this->m_Player2Index != -1) 
		{
			...
			this->m_EventPvPEnter = 0;
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(801), gObj[this->m_Player1Index].Name, gObj[this->m_Player2Index].Name);
			...
			GCPKLevelSend(this->m_Player1Index, 6, 0);
			GCPKLevelSend(this->m_Player2Index, 6, 0);
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(803),gObj[this->m_Player2Index].Name);
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(803),gObj[this->m_Player1Index].Name);
...
bool CEventPvP::CommandEventPvP(LPOBJ lpObj,char* arg) // OK 
{
	...
	if (this->m_RemainTime > 0) 
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(800));
	...
	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(804));
...
void CEventPvP::EventPvPDead(int aIndex,int bIndex) // OK 
{
	...
	{
		this->m_Score1++;
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(802),gObj[this->m_Player1Index].Name,this->m_Score1, this->m_Score2 ,gObj[this->m_Player2Index].Name);
