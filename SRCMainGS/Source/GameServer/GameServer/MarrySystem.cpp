...
void Marry::Accept(LPOBJ lpObj)
{
	...
	if(lpTargetID >= 0)
	{
		...
		if (lpObj->MarryCharacterInfo.Married == 0)
		{
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(580));
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(581),gObj[lpTargetID].Name,lpObj->Name);
