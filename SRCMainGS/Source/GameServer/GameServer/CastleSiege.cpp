...
//season4.5 changed
int g_iNpcUpMaxHP_CGATE[CS_MAX_UPGRADE + 1][0x3] = {
	{2500000,	2,	1000000},
	{3500000,	3,	1000000},
	{5200000,	4,	1000000},
	{1900000,	1,	1000000},
...
//season4.5 changed
int g_iNpcUpMaxHP_CSTATUE[CS_MAX_UPGRADE + 1][0x3] = {
	{2200000,	3,	1000000},
	{3400000,	5,	1000000},
	{5000000,	7,	1000000},
	{1500000,	1,	1000000},
...
int g_iNpcUpRgLevel_CSTATUE[CS_MAX_UPGRADE + 1][0x3] = {
	{1, 3, 5000000},
	{2, 5, 5000000},
	{3, 7, 5000000},
	{0, 0, 5000000},
...
//season4.5 changed??
int g_iCsUserReSpawnArea[CS_MAX_UPGRADE][0x4] = {
	{35,  17, 144,  48},
	{74, 144, 115, 154},
	{35,  17, 144,  48}
};
...
int g_iCastleWallStepAxisY[CS_MAX_UPGRADE] = {
	113, 160, 203
};
...
bool CCastleSiege::LoadData(const std::string& filename)
{
	bool is_open = true;
	bool Success = false;
	...
	if (!Success && !is_open)
	{
		Success = this->LoadTxtData(filename + ".dat");
	}

	return Success;
}

bool CCastleSiege::LoadTxtData(const std::string& filename)
{
	using namespace std;
...
bool CCastleSiege::LoadXmlData(const std::string& filename, bool& is_open)
{
	const char* lpszFileName = filename.c_str();
	...
	pugi::xml_node root = file.child("CastleSiegeEvent");

	if (!root)
	{
		ErrorMessageBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail");
		return false;
	}
	...
	pugi::xml_node child_temp = root.child("CycleSettings");

	if (child_temp)
	{
		this->m_iCastleSiegeCycle = child_temp.attribute("Days").as_int();

		if (this->m_iCastleSiegeCycle <= 0)
		{
			LogAdd(LOG_RED, "[CastleSiege] CCastleSiege::LoadData() - iCastleSiegeCycle <= 0 : %d", this->m_iCastleSiegeCycle);
			return false;
		}

		for (pugi::xml_node child_node = child_temp.child("Cycle"); child_node; child_node = child_node.next_sibling())
		{
			int iSTATE = child_node.attribute("Stage").as_int();

			int iSTART_DAY = child_node.attribute("Day").as_int();

			int iSTART_HOUR = child_node.attribute("Hour").as_int();

			int iSTART_MIN = child_node.attribute("Minute").as_int();
	...
	{
		ErrorMessageBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail: CycleSettings");
		return false;
	}

	child_temp = root.child("NPCSettings");

	if (child_temp)
	{
		for (pugi::xml_node child_node = child_temp.child("NPC"); child_node; child_node = child_node.next_sibling())
		{
			int iNPC_NUM = child_node.attribute("Index").as_int();

			int iNPC_INDEX = child_node.attribute("Number").as_int();

			int iNPC_SIDE = child_node.attribute("Side").as_int();

			int iNPC_DBSAVE = child_node.attribute("DBSave").as_int();

			int iNPC_DF_LEVEL = child_node.attribute("DefenseLevel").as_int();

			int iNPC_RG_LEVEL = child_node.attribute("HPRecLevel").as_int();

			int iNPC_MAXHP = child_node.attribute("MaxHP").as_int();

			int iNPC_SX = child_node.attribute("StartX").as_int();

			int iNPC_SY = child_node.attribute("StartY").as_int();

			int iNPC_DX = child_node.attribute("EndX").as_int();

			int iNPC_DY = child_node.attribute("EndY").as_int();

			int iNPC_DIR = child_node.attribute("Dir").as_int();

			if (iNPC_NUM < 0 || iNPC_INDEX < 0 || iNPC_SIDE < 0 || iNPC_DF_LEVEL < 0
				|| iNPC_RG_LEVEL < 0 || iNPC_MAXHP < 0 || iNPC_SX < 0 || iNPC_SY < 0)
			{
				ErrorMessageBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail: NPCSettings -> (NPC)");
				return false;
			}

			if (iNPC_NUM == 277)
			{
				g_iNpcUpMaxHP_CGATE[CS_MAX_UPGRADE][0] = iNPC_MAXHP;
			}
			if (iNPC_NUM == 283)
			{
				g_iNpcUpMaxHP_CSTATUE[CS_MAX_UPGRADE][0] = iNPC_MAXHP;
			}
			...
			mpNpcDataCheck.insert(std::pair <UINT, int >((MACRO2(iNPC_INDEX) | (MACRO2(iNPC_NUM) << 16)), 0));
	...
	{
		ErrorMessageBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail: NPCSettings");
		return false;
	}

	//---
	child_temp = root.child("DefenseUpgrade");

	if (child_temp)
	{
		pugi::xml_node gate_child;

		const char* type_npc[] = { "Gates", "Statues" };

		for (int n_data = 0; n_data < 2; n_data++)
		{
			gate_child = child_temp.child(type_npc[n_data]);

			if (gate_child)
			{
				int _linecount = 0;

				for (pugi::xml_node child_node = gate_child.child("Defense"); child_node && _linecount < CS_MAX_UPGRADE; child_node = child_node.next_sibling())
				{
					if (n_data)
					{
						g_iNpcUpDfLevel_CSTATUE[_linecount][0] = child_node.attribute("Level").as_int();

						g_iNpcUpDfLevel_CSTATUE[_linecount][1] = child_node.attribute("ReqJewelOfGuardian").as_int();

						g_iNpcUpDfLevel_CSTATUE[_linecount][2] = child_node.attribute("ReqZen").as_int();
					}
					else
					{
						g_iNpcUpDfLevel_CGATE[_linecount][0] = child_node.attribute("Level").as_int();

						g_iNpcUpDfLevel_CGATE[_linecount][1] = child_node.attribute("ReqJewelOfGuardian").as_int();

						g_iNpcUpDfLevel_CGATE[_linecount][2] = child_node.attribute("ReqZen").as_int();
					}
	...
	//---
	child_temp = root.child("HealthUpgrade");
	...
	if (child_temp)
	{
		...
		for (int n_data = 0; n_data < 2; n_data++)
		{
			...
			if (gate_child)
			{
				...
				for (pugi::xml_node child_node = gate_child.child("Health"); child_node && _linecount < CS_MAX_UPGRADE; child_node = child_node.next_sibling())
				{
					if (n_data != 0)
					{
						g_iNpcUpMaxHP_CSTATUE[_linecount][0] = child_node.attribute("Value").as_int();

						g_iNpcUpMaxHP_CSTATUE[_linecount][1] = child_node.attribute("ReqJewelOfGuardian").as_int();

						g_iNpcUpMaxHP_CSTATUE[_linecount][2] = child_node.attribute("ReqZen").as_int();
					}
					else
					{
						g_iNpcUpMaxHP_CGATE[_linecount][0] = child_node.attribute("Value").as_int();

						g_iNpcUpMaxHP_CGATE[_linecount][1] = child_node.attribute("ReqJewelOfGuardian").as_int();

						g_iNpcUpMaxHP_CGATE[_linecount][2] = child_node.attribute("ReqZen").as_int();
					}
	...
	//---
	child_temp = root.child("DefenseSettings");
	...
	if (child_temp)
	{
		...
		for (int n_data = 0; n_data < 2; n_data++)
		{
			...
			if (gate_child)
			{
				...
				{
					if (n_data != 0)
						g_iNpcDefense_CSTATUE[_linecount] = child_node.attribute("Value").as_int();
					else
						g_iNpcDefense_CGATE[_linecount] = child_node.attribute("Value").as_int();
	...
	child_temp = root.child("StatueHealthRecoveryUpgrade");

	if (child_temp)
	{
		int _linecount = 0;
		for (pugi::xml_node child_node = child_temp.child("Recovery"); child_node && _linecount < CS_MAX_UPGRADE; child_node = child_node.next_sibling())
		{
			g_iNpcUpRgLevel_CSTATUE[_linecount][0] = child_node.attribute("Level").as_int();
			g_iNpcUpRgLevel_CSTATUE[_linecount][1] = child_node.attribute("ReqJewelOfGuardian").as_int();
			g_iNpcUpRgLevel_CSTATUE[_linecount][2] = child_node.attribute("ReqZen").as_int();
...
void CCastleSiege::SetState_ENDSIEGE(BOOL bSetRemainMsec)
{
	...
	this->SendAllUserAnyMsg(gMessage.GlobalText(416), 2);
}
...
void CCastleSiege::ProcState_REGSIEGE()
{
	...
	if (iTICK_MSEC >= 1000)
	{
		...
		{
			this->SendAllUserAnyMsg(gMessage.GlobalText(417), 2);
...
void CCastleSiege::ProcState_REGMARK()
{
	...
	if (iTICK_MSEC >= 1000)
	{
		...
		if ((GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000)
		{
			this->SendAllUserAnyMsg(gMessage.GlobalText(418), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
void CCastleSiege::ProcState_IDLE_3()
{
	...
	if (iTICK_MSEC >= 1000)
	{
		...
		{
			this->SendAllUserAnyMsg(gMessage.GlobalText(419), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
void CCastleSiege::ProcState_NOTIFY()
{
	...
	if (iTICK_MSEC >= 1000)
	{
		...
		if ((GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000)
		{
			this->SendAllUserAnyMsg(gMessage.GlobalText(420), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
void CCastleSiege::ProcState_READYSIEGE()
{
	...
	if (iTICK_MSEC >= 1000)
	{
		...
		if ((GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000)
		{
			this->SendAllUserAnyMsg(gMessage.GlobalText(421), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
		if (this->m_iCS_REMAIN_MSEC <= 300000)
		{
			...
			{
				char szBuff[256] = { 0 };
				wsprintf(szBuff, gMessage.GlobalText(422), (this->m_iCS_REMAIN_MSEC / 60000) + 1);
...
void CCastleSiege::ProcState_STARTSIEGE()
{
	...
	if (iTICK_MSEC >= 1000)
	{
		...
		if ((GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000)
		{
			this->SendAllUserAnyMsg(gMessage.GlobalText(423), 2);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
...
int CCastleSiege::CheckCastleSiegeResult()
{
	...
	if (this->m_btIsCastleOccupied == TRUE && strcmp(this->m_szCastleOwnerGuild, "") != FALSE)
	{
		if (strcmp(this->m_szMiddleWinnerGuild, "") == FALSE || strcmp(this->m_szMiddleWinnerGuild, this->m_szCastleOwnerGuild) == FALSE)
		{
			...
			wsprintf(szMsg, gMessage.GlobalText(424), this->m_szCastleOwnerGuild);
			bRETVAL = FALSE;
		}
		else
		{
			...
			wsprintf(szMsg, gMessage.GlobalText(425), this->m_szCastleOwnerGuild);
			bRETVAL = TRUE;
		}
	}
	else
	{
		...
		{
			this->m_btIsCastleOccupied = 0;
			wsprintf(szMsg, gMessage.GlobalText(426));
			bRETVAL = FALSE;
		}
		else
		{
...
void CCastleSiege::SetDbDataLoadOK(BOOL bDbDataLoadOK)
{
