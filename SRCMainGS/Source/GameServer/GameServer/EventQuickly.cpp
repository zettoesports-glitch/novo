	...
	{ 
		...
		if(this->MinutesLeft != minutes)
		{
			this->MinutesLeft = minutes;
				...
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(661),(MinutesLeft+1));
		}

		if(this->m_RemainTime <= 5)
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(662),m_RemainTime);
		}
		...
		if(this->m_RemainTime <= 0)
		{
			    ...
			    this->Clear();
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(664));
...
void CEventQuickly::CommandEventQuickly(LPOBJ lpObj,char* arg) // OK 
{
	...
	if (this->m_RemainTime > 0) 
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(666));
		return;
	}
	...
	if(map < 0 || x <= 0 || y <= 0)
	{
		...
		//gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(665));
		//return;
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(660),MapName);
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(665),x,y);
...
void CEventQuickly::ClickNPC(LPOBJ lpObj) // OK 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(663),lpObj->Name);
