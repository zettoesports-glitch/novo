...
void CTvTEvent::ProcState_EMPTY() // OK
{
	if(this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime*60))
	{
		...
		if((this->AlarmMinSave=(((this->m_RemainTime%60)==0)?((this->m_RemainTime/60)-1):(this->m_RemainTime/60))) != this->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(825),(this->AlarmMinLeft+1));
			//gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(826));
		}
	}

	if(this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GlobalText(827));
		this->NoticeSendToAll(1,gMessage.GlobalText(828),this->m_StandTime);
...
void CTvTEvent::ProcState_STAND() // OK
{
	...
	{
		this->NoticeSendToAll(0,gMessage.GlobalText(830));
		this->SetState(TVT_EVENT_STATE_EMPTY);
		return;
	}
	...
	if(this->m_RemainTime <= 0)
	{
		...
		{
			this->DivisionTeam();
			this->NoticeSendToAll(0,gMessage.GlobalText(829));
...
bool CTvTEvent::Dialog(LPOBJ lpObj, LPOBJ lpNpc)
{
	...
	{
		GCChatTargetSend(lpObj, lpNpc->Index, gMessage.GlobalText(826));
...
bool CTvTEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(831));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(832));
		return 0;
	}
