...
CProtect::CProtect() // OK
{
	...
	this->m_socket[0] = INVALID_SOCKET;

	this->m_socket[1] = INVALID_SOCKET;
...
void CProtect::PackageEnc(void* Buffer, int size, BYTE iKey)
{
	VM_START;

	PBYTE pbyBuffer = (PBYTE)(Buffer);
	for (int n = 4; n < size; n++)
	{
		pbyBuffer[n] ^= iKey + 0x1A;
		pbyBuffer[n] -= 0x69;
	}
...
void CProtect::PackageDec(void* Buffer, int size, BYTE iKey)
{
	VM_START;
...
bool CProtect::ConnectToAuthServer(int index, char* IpAddress, WORD port) // OK
{
#if GAMESERVER_LICENSE == 1
#if(PROTECT_STATE==1)
	...
	if (this->m_socket[index] != INVALID_SOCKET)
	{
		return 0;
	}

	this->m_socket[index] = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	...
	if (connect(this->m_socket[index], (SOCKADDR*)&target, sizeof(target)) == SOCKET_ERROR)
	{
...
bool CProtect::AuthServerDataSend(eAuthServerType ServerType) // OK
{
	...
	memcpy(pMsg.CustomerName, gServerInfo.m_LicenseName, sizeof(pMsg.CustomerName));

	memcpy(pMsg.CustomerHardwareId, gServerInfo.m_CustomerHardwareId, sizeof(pMsg.CustomerHardwareId));

	PackageEnc(&pMsg, sizeof(SDHP_AUTH_SERVER_DATA_SEND), pMsg.EncKey);

	if (send(this->m_socket[0], (char*)&pMsg, pMsg.size, 0) == SOCKET_ERROR)
	{
		if (WSAGetLastError() != WSAEWOULDBLOCK)
		{
			return 0;
...
bool CProtect::AuthServerOnlineSend() // OK
{
	...
	if (send(this->m_socket[1], (char*)&pMsg, pMsg.size, 0) == SOCKET_ERROR)
	{
		if (WSAGetLastError() != WSAEWOULDBLOCK)
		{
			return 0;
...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	if (this->ConnectToAuthServer(0, this->GetEncryptedString((BYTE*)this->HardConnection.AddressIp, this->HardConnection.Size), this->HardConnection.Port) == 0)
	{
		closesocket(this->m_socket[0]);
		this->m_socket[0] = INVALID_SOCKET;
		this->ErrorMessage(1);
		return;
	}
	...
	if (recv(this->m_socket[0], (char*)&this->m_AuthInfo, sizeof(this->m_AuthInfo), 0) != sizeof(SDHP_AUTH_SERVER_DATA_RECV))
	{
		closesocket(this->m_socket[0]);
		this->m_socket[0] = INVALID_SOCKET;
		this->ErrorMessage(3);
	...
	PackageDec((BYTE*)&this->m_AuthInfo, sizeof(SDHP_AUTH_SERVER_DATA_RECV), this->m_AuthInfo.EncKey);
	...
	if (strcmp(this->m_AuthInfo.CustomerName, gServerInfo.m_LicenseName) != 0)
	{
		closesocket(this->m_socket[0]);
		this->m_socket[0] = INVALID_SOCKET;
		this->ErrorMessage(6);
	...
	this->m_EncMain = this->m_AuthInfo.PlanType;

	//LogAdd(LOG_RED, "[AuthServer] 0%X", this->m_EncMain);
...
void CProtect::OnlineAuth() // OK
{
	...
	if (this->ConnectToAuthServer(1, this->GetEncryptedString((BYTE*)this->HardConnection.AddressIp, this->HardConnection.Size), this->HardConnection.Port) == 0)
	{
		closesocket(this->m_socket[1]);
		this->m_socket[1] = INVALID_SOCKET;
	...
	if (recv(this->m_socket[1], (char*)&this->m_AuthOnlineInfo, sizeof(this->m_AuthOnlineInfo), 0) != sizeof(SDHP_AUTH_SERVER_ONLINE_RECV))
	{
		closesocket(this->m_socket[1]);
		this->m_socket[1] = INVALID_SOCKET;
		return;
	...
	PackageDec((BYTE*)&this->m_AuthOnlineInfo, sizeof(SDHP_AUTH_SERVER_ONLINE_RECV), this->m_AuthOnlineInfo.EncKey);
...
inline void CProtect::ErrorMessage(int Erro) // OK
{
#if GAMESERVER_LICENSE == 1
#if(PROTECT_STATE==1)
...
void CProtect::SafeExitProcess() // OK
{
#if GAMESERVER_LICENSE == 1
#if(PROTECT_STATE==1)
...
void CProtect::InsertMemoryMacro(BYTE* data, BYTE* info, int size) // OK
{
#if GAMESERVER_LICENSE == 1
#if(PROTECT_STATE==1)
...
void CProtect::RemoveMemoryMacro(BYTE* data, BYTE* info, int size) // OK
{
#if GAMESERVER_LICENSE == 1
#if(PROTECT_STATE==1)
...
bool CProtect::VerifyMemoryMacro(BYTE* data, BYTE* info, int size) // OK
{
#if GAMESERVER_LICENSE == 1
#if(PROTECT_STATE==1)
...
void CProtect::OpenHardConnection(std::string filename)
{
#if GAMESERVER_LICENSE == 1
#if(PROTECT_STATE==1)
	HANDLE file = CreateFile(filename.c_str(), GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_ARCHIVE, 0);

	if (file != INVALID_HANDLE_VALUE)
	{

		if (GetFileSize(file, 0) != sizeof(SDHP_CONNECTION_SERVER))
		{
			CloseHandle(file);
			return;
		}
		...
		if (ReadFile(file, &this->HardConnection, sizeof(SDHP_CONNECTION_SERVER), &OutSize, 0) == 0)
		{
			CloseHandle(file);
			return;
		}

		for (int n = 0; n < sizeof(SDHP_CONNECTION_SERVER); n++)
		{
			((BYTE*)&this->HardConnection)[n] += (BYTE)(0x95 ^ HIBYTE(n));
			((BYTE*)&this->HardConnection)[n] ^= (BYTE)(0xDA ^ LOBYTE(n));
		}
		CloseHandle(file);
	}
	else
