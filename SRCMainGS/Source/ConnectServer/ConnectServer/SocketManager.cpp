...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	int sendBufSize = MAX_MAIN_PACKET_SIZE * 2; // 2 MB de buffer de envío

	if (setsockopt(this->m_listen, SOL_SOCKET, SO_SNDBUF, (char*)&sendBufSize, sizeof(sendBufSize)) == SOCKET_ERROR)
	{
		LogAdd(LOG_RED, "[SocketManager] setsockopt(SO_SNDBUF) failed with error: %d", WSAGetLastError());
		closesocket(this->m_listen);
		return 0;
	}
...
bool CSocketManager::DataSend(int index, BYTE* lpMsg, int size) // OK
{
	...
	{
#ifdef SOCKET_LAUNCHER
		this->add_message_to_queue(lpIoContext, lpMsg, size);
		this->m_critical.unlock();
		return 1;
#else
...
void CSocketManager::OnSend(int index, DWORD IoSize, IO_SEND_CONTEXT* lpIoContext) // OK
{
	...
	{
#ifdef SOCKET_LAUNCHER
		if (lpIoContext->IoSideBuffer.empty())
		{
		...
		// Obtener el próximo buffer de la cola
		IO_SIDE_BUFFER nextBuffer = lpIoContext->IoSideBuffer.front();
		lpIoContext->IoSideBuffer.erase(lpIoContext->IoSideBuffer.begin());

		memcpy(lpIoContext->IoMainBuffer.buff, nextBuffer.buff, nextBuffer.size);

		lpIoContext->wsabuf.buf = (char*)lpIoContext->IoMainBuffer.buff;
		lpIoContext->wsabuf.len = nextBuffer.size;

		lpIoContext->IoType = IO_SEND;
		lpIoContext->IoSize = nextBuffer.size;
